...
// Latest AIS + 6h tracks + vessel dimensions for Auckland Harbour AIS Viewer.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!latestRes.ok) return jsonResponse({ ok:false, error:"Failed to read ais_latest.", status:latestRes.status, details:await latestRes.text() }, 500);
    ...
    if (mmsis.length > 0) {
      ...
      if (!trackRes.ok) return jsonResponse({ ok:false, error:"Failed to read ais_positions.", status:trackRes.status, details:await trackRes.text() }, 500);
      const trackRows = await trackRes.json();
      for (const row of trackRows) {
        const key = String(row.mmsi);
        if (!tracks[key]) tracks[key] = [];
        if (Number.isFinite(Number(row.lat)) && Number.isFinite(Number(row.lon))) tracks[key].push([Number(row.lat), Number(row.lon)]);
      }
      for (const vessel of vessels) {
        const key = String(vessel.mmsi);
        if (!tracks[key] || tracks[key].length === 0) tracks[key] = [[Number(vessel.lat), Number(vessel.lon)]];
      }
    }

    return jsonResponse({ ok:true, provider:"AISStream via Supabase", count:vessels.length, trackWindowMinutes:minutes, bounds:{latMin,latMax,lonMin,lonMax}, vessels, tracks });
  } catch (error) {
